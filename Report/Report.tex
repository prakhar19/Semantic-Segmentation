...
For example, the Figure~\ref{fig:semantic_segmentation} has been segmented 
...
define the desired segmentation categories, as an image with infinte detail 
...
Fully Convolutional Networks (FCNs) are what one gets on building a neural 
network using only convolutional-type layers. They keep the spatial information 
of an image intact.

\begin{figure}[!h]
    \includegraphics[width=\linewidth]{fcn.png}
    \caption{Representation of a FCN (Source: J. Long et al, 2015\cite{long2015fully})}
...
we use the set of finely annotated images from the left cam-era.
...
We use a hybrid architecture of a Fully Convolutional Network, a 
Deconvolution Network, and skip connections.
...
\appendices
\section{Model Architecture - Cityscapes Dataset}

\begin{figure}[!hp]
    \includegraphics[height=0.9\textheight]{cityscapes-model.png}
